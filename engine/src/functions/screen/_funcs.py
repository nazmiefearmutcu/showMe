...
def _apply_screen_query(rows: list[dict[str, Any]], query: str) -> list[dict[str, Any]]:
    if not query.strip():
        return list(rows)
    from src.functions.equity.eqs import filter_dataframe
...
def _rank_most_active_rows(rows: list[dict[str, Any]], sort_key: str) -> list[dict[str, Any]]:
    ...
    if sort_key in {"volume", "vol"}:
        key = lambda row: float(row.get("volume") or 0)
    elif sort_key in {"abs_change", "change", "mover"}:
        key = lambda row: abs(float(row.get("change_pct") or 0))
    elif sort_key in {"activity", "score"}:
        key = lambda row: float(row.get("activity_score") or 0)
    else:
        key = lambda row: float(row.get("dollar_volume") or 0)
