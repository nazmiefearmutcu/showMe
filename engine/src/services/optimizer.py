...
def _solve_long_only(mu: np.ndarray, cov: np.ndarray, target_return: float | None = None,
                     iters: int = 4000, lr: float = 0.05, seed: int = 0) -> np.ndarray:
    ...
    for _ in range(iters):
        ...
        if target_return is None:
            # Min variance + return-weighted bias
            grad = grad_var - 0.0001 * mu
        else:
            # Lagrange-ish penalty for target return
            penalty = (w @ mu - target_return)
...
def max_sharpe(returns: pd.DataFrame, *, risk_free: float = 0.0,
    ...
    if allow_short:
        ...
        w = inv @ excess
        s = float(np.ones_like(w) @ w)
        w = w / s if s != 0 else _normalize(np.ones_like(w))
    else:
        # Random search refined by long-only PGD on Sharpe surrogate
