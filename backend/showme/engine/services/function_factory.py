...
from showme.engine.core.base_data_source import DataRouter, BaseDataSource
...
class FunctionFactory:
    ...
    def _build(self) -> None:
        ...
        # Equity
        from showme.engine.data_sources.equity.yfinance_adapter import YFinanceAdapter
        from showme.engine.data_sources.equity.finnhub_adapter import FinnhubAdapter
        from showme.engine.data_sources.equity.alphavantage_adapter import AlphaVantageAdapter
        from showme.engine.data_sources.equity.polygon_adapter import PolygonAdapter
        from showme.engine.data_sources.equity.eodhd_adapter import EODHDAdapter
        from showme.engine.data_sources.equity.stooq_adapter import StooqAdapter
        from showme.engine.data_sources.equity.sec_edgar_adapter import SECEdgarAdapter
        from showme.engine.data_sources.equity.sec_13f_adapter import SEC13FAdapter
        from showme.engine.data_sources.equity.seekingalpha_adapter import SeekingAlphaAdapter
        from showme.engine.data_sources.equity.finra_adapter import FINRAAdapter
        from showme.engine.data_sources.equity.sec_efts_adapter import SECFullTextSearch
        # Crypto
        from showme.engine.data_sources.crypto.coingecko_adapter import CoinGeckoAdapter
        from showme.engine.data_sources.crypto.cryptocompare_adapter import CryptoCompareAdapter
        from showme.engine.data_sources.crypto.ccxt_failover_adapter import CCXTFailoverAdapter
        # Macro
        from showme.engine.data_sources.macro.fred_adapter import FREDAdapter
        from showme.engine.data_sources.macro.worldbank_adapter import WorldBankAdapter
        from showme.engine.data_sources.macro.imf_adapter import IMFAdapter
        from showme.engine.data_sources.macro.oecd_adapter import OECDAdapter
        from showme.engine.data_sources.macro.tradingeconomics_adapter import TradingEconomicsAdapter
        from showme.engine.data_sources.macro.cme_fedwatch_adapter import CMEFedWatchAdapter
        from showme.engine.data_sources.macro.damodaran_adapter import DamodaranAdapter
        # FX
        from showme.engine.data_sources.fx.ecb_adapter import ECBAdapter
        from showme.engine.data_sources.fx.exchangerate_host_adapter import ExchangerateHostAdapter
        # Commodity
        from showme.engine.data_sources.commodity.eia_adapter import EIAAdapter
        # Bond
        from showme.engine.data_sources.bond.ustreasury_adapter import USTreasuryAdapter
        from showme.engine.data_sources.bond.treasury_auctions_adapter import TreasuryAuctionsAdapter
        # News
        from showme.engine.data_sources.news.gdelt_adapter import GDELTAdapter
        from showme.engine.data_sources.news.rss_adapter import RSSAdapter
        from showme.engine.data_sources.news.finnhub_news_adapter import FinnhubNewsAdapter
        # Alt
        ...
        from showme.engine.data_sources.alt.etherscan_adapter import EtherscanAdapter
        from showme.engine.data_sources.alt.mempool_adapter import MempoolAdapter
        from showme.engine.data_sources.alt.notion_adapter import NotionAdapter
        from showme.engine.data_sources.alt.granola_adapter import GranolaAdapter
        from showme.engine.data_sources.alt.polymarket_adapter import PolymarketAdapter
        # Reference
        from showme.engine.data_sources.reference.openfigi_adapter import OpenFIGIAdapter
...
def _ensure_functions_registered() -> None:
    ...
    import importlib
    import pkgutil
    import showme.engine.functions as fns_pkg

    for _, modname, ispkg in pkgutil.walk_packages(fns_pkg.__path__, prefix="showme.engine.functions."):
        if not ispkg:
            try:
